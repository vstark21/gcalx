...
def init(
    force: Annotated[
        bool, typer.Option("--force", "-f", help="Re-authenticate even if already logged in.")
    ] = False,
) -> None:
    ...
    if existing and not force:
        ...
        console.print(
            "[muted]To re-authenticate, run:[/muted]"
        )
        console.print("[muted]  gcalx init --force[/muted]")
        return
...
def today(
    ...
    from gcalx.auth import build_calendar_service, build_tasks_service
    ...
    cfg, cal_client, cache, console = get_deps(
